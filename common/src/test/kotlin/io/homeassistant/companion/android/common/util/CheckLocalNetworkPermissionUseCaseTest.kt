...
class CheckLocalNetworkPermissionUseCaseTest {
    ...
    private lateinit var originalSdkVersionProvider: SdkVersionProvider
    ...
    fun setUp() {
        ...
        originalSdkVersionProvider = sdkVersion
        stubSystemSdkVersion(currentSdk = API_WITH_LOCAL_NETWORK_PERMISSION)
    }

    @AfterEach
    fun tearDown() {
        sdkVersion = originalSdkVersionProvider
    ...
    private fun stubSystemSdkVersion(currentSdk: Int) {
        sdkVersion = SdkVersionProvider { api -> currentSdk >= api }
    }
